...
> 🇬🇧 English documentation: [README.md](README.md) · Mağaza metinleri (EN+TR): [`store/STORE_LISTING.md`](store/STORE_LISTING.md) · Gizlilik: [`PRIVACY_POLICY.md`](PRIVACY_POLICY.md)

## Nasıl çalışır?

1. **Anında engelleme** — Bilinen 140'tan fazla reklam/izleme ağı `rules_static.json`
   içindeki `declarativeNetRequest` kurallarıyla, AI'a hiç sorulmadan tarayıcı motorunda
   engellenir.
...
4. **Dinamik kural** — Reklam sayılan alan adları için dinamik engelleme kuralı eklenir;
   engelleme JS'te değil, ağ katmanında olur (performans kaybı yok). Kurallar **yalnızca
   üçüncü taraf** istekleri keser — bir domain kendi sitesinde asla engellenmez.
...
- API anahtarı **yalnızca tarayıcıda** saklanır (`chrome.storage.local`) — senkronize edilmez,
  loglanmaz, paylaşılmaz. Eklentinin arka uç sunucusu yoktur; geliştiriciler hiçbir veri almaz.
...
4. Ayarlar sayfası otomatik açılır: sağlayıcınızı seçip API anahtarınızı girin,
   "Anahtarı test et" ile doğrulayın.

## Chrome Web Store yayını

- Yayın paketi: `bash scripts/build.sh` → `dist/sentinel-aidblock-v<versiyon>.zip`
- Mağaza metinleri (EN+TR), izin gerekçeleri ve yayın kontrol listesi:
...
sayısı birkaç düzineyi geçmez; ilk günlerden sonra neredeyse sıfıra iner.
...
| `rules_static.json` | Temel engel listesi (148 kural, AI'sız, anında) |
| `content.js` | Kozmetik filtreleme (genel + AI seçicileri, cache'li, çalışma anı güvenli) |
...
| `scripts/build.sh` | Yayın zip'i üretir (tarih-saat versiyonlama) |
